...
import net.dreamlu.mica.ip2region.xdb.Searcher;
...
public class SearcherPool {
	...
	public static SearcherPool create(final Config config) throws IOException {
		return new SearcherPool(config).init();
	}

	public static SearcherPool create(final Config config, boolean fair) throws IOException {
		return new SearcherPool(config, fair).init();
	}

	protected SearcherPool(Config config) {
		this(config, false);
	}
